...
const VECTRACE = (() => {
  ...
  const PRESETS = {
    ...
    pixel: { label: 'Pixel Art', mode: 'exact', colors: 64, tolerance: 0, cornerAngle: 15, minArea: 1, curves: false },
    pixelbw: { label: 'Pixel Art B&W', mode: 'bw', threshold: 128, tolerance: 0, cornerAngle: 15, minArea: 1, curves: false, ignoreWhite: true },
  };
  ...
  // Vertices the simplifier is not allowed to give away. Neighbouring regions
  // trace the same crack in opposite directions, so pinning both of them to
  // these vertices — and only these — makes the shared run simplify and fit to
  // identical geometry on both sides. Without that, every boundary is
  // approximated twice, the two answers disagree by a fraction of a pixel, and
  // artboard shows through in the difference. Two reasons to pin, two bits:
  //   PIN_JUNCTION — three or more regions meet here, so several loops pass
  //     through this point and all of them have to keep it to stay in step.
  //   PIN_EDGE — the vertex is on the image border. That border is an
  //     artificial cut, not an edge of the artwork: nothing is traced beyond
  //     it to take up what this boundary gives away, so a corner shaved off
  //     there is artboard showing at the edge of the art for good. It is also
  //     free to pin, since a border run is straight and collapses to its two
  //     ends anyway. A transparent boundary is deliberately NOT pinned — that
  //     is a real outline of the artwork and gets smoothed like any other,
  //     which is what the tolerance control is for.
  const PIN_JUNCTION = 1, PIN_EDGE = 2;
  ...
  function junctionMap(labels, w, h) {
    ...
    for (let y = 0; y <= h; y++) {
      for (let x = 0; x <= w; x++) {
        ...
        if (n >= 3 || (tl === br && tr === bl && tl !== tr)) f |= PIN_JUNCTION;
        if (x === 0 || y === 0 || x === w || y === h) f |= PIN_EDGE;
        j[x + y * stride] = f;
      }
    }
    return j;
    ...
    for (const s0 of starts) {
      for (;;) {
        ...
        for (;;) {
          ...
          const corner = dir !== last;
          const f = junction ? junction[v] : 0;
          const pin = ((f & PIN_JUNCTION) || (corner && (f & PIN_EDGE))) ? 1 : 0;
          if (corner || pin) { pts.push([v % stride, (v / stride) | 0]); fix.push(pin); }
          last = dir;
  ...
  // Ordering that depends only on where a point *is*, never on which way the
  // run is being walked. Both sides of a shared boundary see the same points
  // in opposite order, so every "pick the extreme point" decision in this file
  // breaks near-ties with this — otherwise the two sides split the run in
  // different places and their simplifications drift apart by a fraction of a
  // pixel. On a pixel grid those ties are the common case, not a corner case.
  function lexLess(a, b) { return a[1] !== b[1] ? a[1] < b[1] : a[0] < b[0]; }

  // Distances either side computes for the same point differ in the last bits
  // (different chord origin, negated deltas), so ties need slack to register.
  function tieSlack(v) { return 1e-9 * (1 + Math.abs(v)); }
  ...
  function rdpOpen(pts, tol) {
    ...
    while (stack.length) {
      ...
      for (let i = a + 1; i < b; i++) {
        ...
        // cross²/L² rather than reconstructing the foot of the perpendicular:
        // on integer grid points the cross product is exact, so a collinear
        // point measures exactly 0 (it must drop at tolerance 0) and the two
        // walk directions get bit-identical distances instead of answers that
        // differ in the last few bits.
        const dot = ux * dx + uy * dy;
        let d2;
        if (L2 <= 0 || dot <= 0) d2 = ux * ux + uy * uy;
        else if (dot >= L2) { const vx = ux - dx, vy = uy - dy; d2 = vx * vx + vy * vy; }
        else { const cr = ux * dy - uy * dx; d2 = cr * cr / L2; }
        const slack = tieSlack(bd);
        if (d2 > bd + slack) { bd = d2; best = i; }
        else if (d2 > bd - slack && lexLess(pts[i], pts[best])) { best = i; if (d2 > bd) bd = d2; }
      }
  ...
  // An anchor chosen from the point set alone, never from wherever the contour
  // walk happened to start, so a loop and its reversed twin agree on it.
  ...
  function simplifyLoop(pts, fix, tol) {
    const n = pts.length;
    if (n < 3) return { pts: pts.slice(), fix: fix ? Array.from(fix) : new Array(n).fill(0) };
    const anchors = [];
    if (fix) for (let i = 0; i < n; i++) if (fix[i]) anchors.push(i);
    // Anchors may only be things the neighbour sharing this boundary also
    // knows about. Junctions qualify; a second anchor invented from this
    // loop's own shape does not — it would cut the shared run somewhere the
    // other side does not, and the two simplifications drift apart there. With
    // a single junction the span simply wraps the whole loop back to it, and
    // with none the anchor comes from the point set (identical on both sides).
    const pinned = anchors.length > 0;
    if (!pinned) anchors.push(canonicalStart(pts));
    const outPts = [], outFix = [];
    for (let k = 0; k < anchors.length; k++) {
      const a = anchors[k], b = anchors[(k + 1) % anchors.length];
      const run = [pts[a]];
      for (let i = (a + 1) % n; ; i = (i + 1) % n) { run.push(pts[i]); if (i === b) break; }
      const r = rdpOpen(run, tol);
      for (let i = 0; i < r.length - 1; i++) {
        outPts.push(r[i]);
        outFix.push(i === 0 && pinned ? 1 : 0);
      }
    }
    // A loop that simplifies below three points has been flattened to a line
    // and gets dropped downstream. Do NOT fall back to the raw points here:
    // this is a whole-loop decision, and the neighbour sharing these arcs makes
    // it separately. When only one of them backs out, the two descriptions of
    // the same boundary stop matching and the gap reopens. A zero-area loop
    // costs nothing — the arcs its neighbours kept still tile the plane.
  ...
  function maxError(pts, u, bz) {
    ...
    for (let i = 1; i < pts.length - 1; i++) {
      ...
      const slack = tieSlack(max);
      if (d > max + slack) { max = d; at = i; }
      else if (d > max - slack && lexLess(pts[i], pts[at])) { at = i; if (d > max) max = d; }
    }
    if (at < 0) return [0, (pts.length / 2) | 0]; // nothing between the ends
  ...
  function fitPath(pts, opts, fix) {
    ...
    const runs = [];
    // A loop with no corner at all is one smooth run whose ends must meet
    // tangent-continuously. Every other run ends on a corner, so its tangents
    // are free — including a run that happens to start and end at the same
    // vertex, which is what a pinched region's lobe looks like. Deciding that
    // by identity rather than by corner-ness made a lobe fit differently from
    // the neighbour tracing the same arc as a whole loop.
    const smoothSeam = corners.length === 0;
    if (corners.length < 2) {
      const s = smoothSeam ? canonicalStart(pts) : corners[0];
      const run = [];
    ...
    for (const run of runs) {
      ...
      if (smoothSeam && m > 3) { // keep tangent continuity across the seam
    ...
    // the last run lands back on the start anchor; Z already draws that edge
    const tail = cmds[cmds.length - 1];
    if (cmds.length >= 4 && tail[0] === 'L'
      && Math.abs(tail[1] - cmds[0][1]) < 1e-9 && Math.abs(tail[2] - cmds[0][2]) < 1e-9) cmds.pop();
    cmds.push(['Z']);
    return cmds;
  }
  ...
  function trace(bmp, opts) {
    ...
    for (let l = 0; l < reg.count; l++) {
      ...
      //
      // Nest first, on the raw pixel loops, where containment is exact. A
      // heavily simplified outer can pull inside its own hole, and a hole that
      // then fails to find a parent gets subtracted from the wrong subpath —
      // which reads as a bite taken out of the region.
  ...
  return {
    ...
    contours, junctionMap, polyArea, nestLoops, pointInPoly, dropCollinear,
