...
function newRes() {
  return { seps: [], sepIdx: new Map(), gs: new Map(), fonts: false };
}
...
// One shape, inside its own q/Q so color and overprint stay local.
function shapeOps(shape, res, fillCol, strokeCol) {
  ...
  if (strokeCol) {
    lines.push(colorOps(strokeCol, true, res));
    lines.push(fmt(shape.strokeWidth || 1) + ' w');
  }
  lines.push(...pathOps(shape.subpaths));
  lines.push(paintOp({ fill: fillCol, stroke: strokeCol, fillRule: shape.fillRule }));
  lines.push('Q');
  return lines;
}
